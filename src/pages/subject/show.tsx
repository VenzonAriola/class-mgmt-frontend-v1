...
const SubjectsShow = () => {
  ...
  // Extract classes and teachers from subject details
  ...
  const teachers = useMemo(() => {
    const uniqueTeachers = new Map<string, SubjectUser>();
    classes.forEach((cls) => {
      if (cls.teacher) {
        uniqueTeachers.set(cls.teacher.id, {
          id: cls.teacher.id,
    ...
    <ShowView className="class-view space-y-6">
      ...
      <Card className="hover:shadow-md transition-shadow">
        ...
        <CardContent className="space-y-2">
          {details.department ? (
            <>
              <Link
                to={`/departments/show/${details.department.id}`}
                ...
                <tbody>
                  {classes.map((cls) => (
                    <tr key={cls.id} className="border-b hover:bg-muted/50">
                      ...
                      <td className="px-4 py-2">{cls.capacity ?? "N/A"}</td>
                      <td className="px-4 py-2">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="hover:shadow-md transition-shadow">
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground">
              Students are enrolled in classes, not directly in subjects.
            </p>
          </CardContent>
        </Card>
      </div>
